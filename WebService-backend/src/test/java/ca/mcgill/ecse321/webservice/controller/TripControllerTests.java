...
public class TripControllerTests {
	...
	public void setMockOutput(){
		when(tripDAO.getTrip(ArgumentMatchers.anyLong())).thenAnswer( (InvocationOnMock invocation) -> {
			...
			if (arg == -1) {
				...
				Optional<Trip> trip = Optional.of(new Trip("foo", "foo", 0, true, null, null, 0, 0, null, false, null));
				return trip;
			} else {
				return null;
			}
	...
	public void getExistingTrip() {		
		...
		Optional<Trip> expected = Optional.of(new Trip("foo", "foo", 0, true, null, null, 0, 0, null, false, null));
